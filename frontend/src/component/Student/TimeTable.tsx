...
const TimeTable: React.FC<TimeTableProps> = ({ userRole, onlyDay }) => {
  ...
  const handleAddTimeSlot = () => {
    ...
    if (!newTime.match(/^\d{1,2}:\d{2}–\d{1,2}:\d{2}\s?(AM|PM)$/i)) {
      alert("Please enter a valid time (e.g., 2:00–3:00 PM)");
      return;
    }
      ...
      {isStudent && (
        <Card className="shadow-md border-purple-200">
          <CardContent className="p-4 flex items-center gap-3 text-purple-700">
            ...
            <div>
              <p className="font-semibold">{currentUserRole === "student" ? "Student View" : "Parent View"}</p>
              <p className="text-sm">
                {currentUserRole === "student"
                  ? "You can view the timetable but cannot edit it. Contact an admin for changes."
                  : "You can view your child's timetable. Contact the admin for changes."}
              </p>
            </div>
          </CardContent>
        </Card>
